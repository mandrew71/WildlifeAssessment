...
occSummary <- all_species %>%           # create a new object to hold the summary
  filter(Species == ourSpecies,
         Sign %in% c("Snare", "Trail", "Camp", "Sign of passage", "Trail","Cartrige", "Footprint",
         "Sighting", "Gunshot")) %>%     # Only our hunters activities
...
saveRDS(huntersData, file="Data/huntersData.rds") 
...
#get the data
huntersData <- readRDS("Data/huntersData.rds")
...
max(huntersData$Elevmean)

M  <- mean(huntersData$Elevmean)   # Mean of original elevation data
SD  <- sd(huntersData$Elevmean)    # SD of original elevation data
...
(trendPlot <- ggplot(plotData, aes(Elevmean)) +                             # Create the plot
    ...
    labs(x="Elevation (m a.s.l.)",                                                  # Label the x-axis
         ...
         title="Occurrence probability of Hunting activities")) 
#Lovely. Let's save that graph
ggsave(trendPlot, file="Plots/ElevTrendHunters.png", width=8, height=8)
